...
date = current_date.strftime("%b") + '_' + str(current_date.day)
...
def collect_data(envr, policy, buffer, dataset, agent, ep_counter, params):
    ...
    while not np.equal(time_step.step_type, 2):
        ...
        if np.random.random() < epsilon:
            action_no = np.random.randint(0, 5)
...
def dqn_train(params):
    ...
    if not os.path.exists('trained_models/' + mname):
        if not os.path.exists('trained_models'):
            os.mkdir('trained_models')
        os.mkdir('trained_models/' + mname)
        os.mkdir('trained_models/' + mname + '/plots')

    params.print_params('trained_models/' + mname)
    ...
    dataset = replay_buffer.as_dataset(
        ...
        num_steps=params.num_steps, single_deterministic_pass=False).prefetch(params.prefetch)
    ...
    while ep_counter < episodes:
        ...
        if ep_counter % params.DQN_policy_store_frequency == 0 and ep_counter >= 1:
            policy_dir = os.path.join('trained_models', mname, mname + '_ep_' + str(ep_counter))
    ...
    savemat('trained_models/'+mname+'/'+mname+'.mat', mat_dict)
    ...
    plt.xlabel("Episodes")
    plt.grid()
    plt.savefig('trained_models/'+mname+'/plots/'+mname+'_return.png', dpi=600)
    ...
    plt.plot(avg_losses)
    plt.grid()
    plt.savefig('trained_models/'+mname+'/plots/'+mname+'_loss.png', dpi=600)
    # plt.show()
