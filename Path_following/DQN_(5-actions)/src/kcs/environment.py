...
class ship_environment(py_environment.PyEnvironment):
    ...
    def __init__(self, train_test_flag=0,
        ...
        if train_test_flag == 0:
            ...
            self.ship_path, = self.ax.plot(0,0, c = 'blue', label = 'Ship Path')
            self.predefined, = self.ax.plot(0,0, c='gray',marker = 'd', linestyle='dashed', label = 'Predefined Path')
            # plt.scatter(self.x[-1], self.y[-1], color='red', label='Destination', zorder=5, s=100, alpha=0.6)
            plt.scatter(self.x_goal, self.y_goal, color='green', label='Start', zorder=5, s=100, alpha=0.6)
            plt.legend()
            plt.grid(True)
            plt.xlabel("X-axis")
            plt.ylabel("Y-axis")
            self.ax.set_xlim(left=-40, right = 40)
            
            self.ax.set_ylim(bottom=-40, top = 40)
    ...
    def _step(self, action_no):
        ...
        self.obs_state[5] = psi             # Hearandomding angle
    ...
    def _reset(self):
        ...
        if self.train_test_flag == 0:
            ...
            if self.counter>0 and self.counter % 10 ==0:
                self.render()
    ...
    def render(self, mode="human"):
        self.ax.set_xlim(left=min(self.x_goal,0)-10, right = max(self.x_goal,0)+10)
        self.ax.set_ylim(bottom=min(self.y_goal,0)-10, top = max(self.y_goal,0)+10)
        self.ship_path.set_data(self.x_traj, self.y_traj)
        self.predefined.set_data([0,self.x_goal],[0,self.y_goal])
        self.fig.canvas.draw()
        self.fig.canvas.flush_events()
        print('Episode count:',self.counter)
