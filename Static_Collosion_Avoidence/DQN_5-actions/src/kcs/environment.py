...
class ship_environment(py_environment.PyEnvironment):
    ...
    def __init__(self, train_test_flag=0,
        ...
        self._observation_spec = array_spec.BoundedArraySpec(shape=(7,), dtype=np.float32, minimum=[-20, -np.pi, 0, -1,0,-np.pi,0],
                                                             maximum=[20, np.pi, 50, 1,100,np.pi,1], name='observation')
    ...
    def _step(self, action_no):
        ...
        ob_x, ob_y = self.obs_x,self.obs_y
        psi_obs = np.arctan2(ob_y-y,ob_x-x)
        angle_obs = course_angle - psi_obs
        angle_obs = (angle_obs + np.pi)%(2*np.pi) - np.pi
        dist_obs = np.sqrt((ob_y-y)**2 + (ob_x-x)**2)
        if self.train_test_flag == 1:
            self.course_ang_err_traj.append(course_angle_err)
         
        # REWARD FUNCTIONS 
        R1 = -0.3 * abs(cross_track_error)
        R2 = -0.25 * abs(course_angle_err)
        R3 = -0.25 * distance
        if dist_obs < 2*self.obsize:
            R4 = -0.2 * dist_obs
        else:
            R4 = -0.2 * (1/dist_obs)
        reward = R1 + R2 + R3 + R4
        ...
        if self.train_test_flag == 1:
            ...
            self.reward_04.append(R4)
            self.total_reward.append(reward)

        observation = [cross_track_error, course_angle_err, distance, r, dist_obs,angle_obs,self.obsize]
    ...
    def _reset(self):
        ...
        if self.train_test_flag == 0:
            ...
            a=np.random.random()
            if a>0.4:     ## Case 1: obstacle in front
                for i in range(self.num_obs):
                    f = 0.5 * np.random.random() + 0.25
                    d_obs = f * radius
                    random_angle = random_theta
                    self.obs_x.append(f * x_goal)
                    self.obs_y.append(f * y_goal)
            else:   ## Case 2: Obstacle anywhere else
                for i in range(self.num_obs):
                    f = 0.5 * np.random.random() + 0.25
                    d_obs = f * radius
                    random_angle = 2*np.pi* np.random.random()
                    self.obs_x.append(d_obs * np.cos(random_angle))
                    self.obs_y.append(d_obs * np.sin(random_angle))
            ...
            observation = np.array([0, course_angle_err, radius, 0,d_obs,random_angle,self.obsize], dtype=np.float32)
            ...
            d_obs=np.sqrt(self.obs_x**2 +self.obs_y**2)
            psi_obs=np.arctan2(self.obs_y,self.obs_x)
            ...
            observation = np.array([0, course_angle_err, dist_to_goal, 0,d_obs,psi_obs,self.obsize], dtype=np.float32)
